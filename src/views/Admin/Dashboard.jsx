    ...
    <div className="min-h-screen bg-tl-paper pt-32 px-6 pb-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-12">
            <div>
                <h1 className="font-serif text-4xl text-tl-soot">
                    Admin Dashboard 
                    <span className="ml-4 text-lg text-tl-soot/40 font-sans font-normal">
                        ({products.length} items)
                    </span>
                </h1>
                <p className="text-tl-soot/60 mt-2 font-light">Manage your tea inventory.</p>
            </div>
            <Link 
                href="/admin/products/new" 
                className="flex items-center gap-2 bg-tl-matcha text-white px-6 py-3 rounded-full text-xs uppercase tracking-widest font-bold hover:bg-tl-forest transition-colors"
            >
        ...
        {/* Product Table */}
        <div className="bg-white rounded-xl shadow-sm border border-tl-soot/10">
            {loading ? (
                <div className="p-12 text-center text-tl-soot/40 italic">Loading inventory...</div>
            ) : products.length === 0 ? (
                <div className="p-12 text-center text-tl-soot/40 italic">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-tl-soot/10 bg-white">
                                ...
                                <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                                    <td className="p-4 md:p-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <span className="font-medium text-tl-soot">{product.name}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="p-4 md:p-6 text-sm text-gray-600 flex items-center gap-2">
                                        <Package size={14} className="text-tl-matcha" />
                                        {product.stock}
                                    </td>
                                    <td className="p-4 md:p-6 text-right">
                                        <div className="flex justify-end gap-3">
                                            <Link href={`/admin/products/edit/${product._id}`} className="p-2 text-gray-400 hover:text-tl-matcha transition-colors">
                                                <Edit2 size={16} />
                                            </Link>
